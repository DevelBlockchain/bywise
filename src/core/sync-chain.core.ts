    ...
    async run() {
        if (this.coreContext.network.web3.network.isConnected) {
            ...
            if (nextBlock) {
                ...
                this.coreContext.blockTime = parseInt((await this.coreContext.configsProvider.getByName(this.coreContext.blockTree, currentBlock.hash, currentBlock.height, 'blockTime')).value);
