...
export class VirtualMachineProvider {
  ...
  async blockchainCommand(ctx: SimulateDTO, cmd: CommandDTO): Promise<void> {
    ...
    if (cmd.name == 'vote-block') {
      ...
      const votes = await this.applicationContext.database.VotesRepository.findByChainAndHeightAndFrom(ctx.blockTree.chain, height, from);
