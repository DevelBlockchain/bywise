...
const ERC1155Code = fs.readFileSync('./BST1155/BST1155.js', 'utf8');
...
const main = async (): Promise<void> => {
    ...
    const tx = await web3.transactions.buildSimpleTx(
        ...
        '0',
        TxType.TX_CONTRACT,
        { contractAddress, code: ERC1155Code }
    );

    console.log("Deploying BST1155 contract at address", contractAddress);
    const output = await web3.transactions.sendTransaction(tx);
    console.log('sendTransaction', tx.hash, output);
    await waitSend(tx);

    console.log("BST1155 contract deployed at:", contractAddress);
}
