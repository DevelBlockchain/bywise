...
const ERC20Code = fs.readFileSync('./BST20/BST20.js', 'utf8');
...
const main = async (): Promise<void> => {
    ...
    const tx = await web3.transactions.buildSimpleTx(
        ...
        '0',
        TxType.TX_CONTRACT,
        { contractAddress, code: ERC20Code }
    );

    console.log("Deploying BST20 contract at address", contractAddress);
    const output = await web3.transactions.sendTransaction(tx);
    console.log('sendTransaction', tx.hash, output);
    await waitSend(tx);

    console.log("BST20 contract deployed at:", contractAddress);
}
